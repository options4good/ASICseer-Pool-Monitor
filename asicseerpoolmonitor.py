...
state = {
    ...
    "block_hash": "Unknown",
    "reward": "0",
    "runtime": "00:00:00:00", # Format: DD:HH:MM:SS
    "total_users": 0,
    "total_workers": 0,
...
def parse_line(line):
    ...
    if "Pool:{" in line:
        try:
            ...
            if "runtime" in data: 
                ...
                state["total_users"] = data.get("Users", state.get("total_users", 0))
                state["total_workers"] = data.get("Workers", state.get("total_workers", 0))
    ...
    if "authorized client" in line:
        ip_match = re.search(r'client \d+ ([\d\.]+)', line)
        worker_match = re.search(r'worker\s+([a-zA-Z0-9:]+\.[a-zA-Z0-9_-]+)', line)
        user_match = re.search(r'as user\s+([a-zA-Z0-9:]+)', line)
        
        ip_val = ip_match.group(1) if ip_match else "NA"
        work_val = worker_match.group(1).split('.')[-1] if worker_match else "NA"
